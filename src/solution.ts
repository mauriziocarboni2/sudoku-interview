...
const isValidValue = (y: number, x: number, box: number, value: CellValue, cacheMap: CacheMap): boolean =>
    // ToDo: complete this function
    false;

// This function checks if the combination is valid
const isValidSudokuCombination = (board: CellValue[][], cacheMap: CacheMap): boolean => {
    for (let y=0; y<9; y++) {
        for (let x=0; x<9; x++) {
            if (board[y][x] === ".") {
 ...
 */
export function solveSudoku(board: CellValue[][]): void {
    const cacheMap: CacheMap = {
        row: Array.from({ length: 9 }).map(_ => new Map<CellValue, boolean>()),
        col: Array.from({ length: 9 }).map(_ => new Map<CellValue, boolean>()),
        box: Array.from({ length: 9 }).map(_ => new Map<CellValue, boolean>()),
    }
